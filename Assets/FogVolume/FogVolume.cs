 ...
 * Fixed noise collision bug
 * Added spherical noise fade

 *  ---- v 2.0a8 ----

 * Fixed shader warning related with loop unrolling
 * */
...
public class FogVolume : MonoBehaviour
{
    ...
    void SetCameraDepth()
    {
        if (ForwardRenderingCamera != null)
        {
            for(int i = 0;i< ForwardRenderingCamera.Length;i++)
            {
                ForwardRenderingCamera[i].depthTextureMode |= DepthTextureMode.Depth;
            }
        }
            
    }

    public void setCamDepth(Camera cam)
    {
    ...
    void Awake()
    {
        ...
        UpdateBoxMesh();
       // Debug.Log(SystemInfo.supports3DTextures);
    }
    ...
    void Update()
    {
        ...
        fogVolumeScale.x = Mathf.Max(fogVolumeScale.x, 1);
        fogVolumeScale.y = Mathf.Max(fogVolumeScale.y, 1);
        fogVolumeScale.z = Mathf.Max(fogVolumeScale.z, 1);
    ...
    void ToggleKeyword()
    {
        if (FogMaterial)
        {
            ...
            if (EnableNoise && Sun && _ShadeNoise && SystemInfo.supports3DTextures)
